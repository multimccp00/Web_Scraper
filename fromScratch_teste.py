...
# Function to translate and convert to uppercase
def translate_and_uppercase(text):
    if text:
        translated_text = translator.translate(text, src='pt', dest='en').text
        return translated_text.upper()  # Convert the translated text to uppercase
    return 'No Data'
...
def scrape_cars(page_number):
    ...
    if response.status_code == 200:
        ...
        for car_section in car_sections:
            ...
            # Use html.unescape() to handle Unicode characters and translate to English, then convert to uppercase
            car = Car(
                title=translate_and_uppercase(html.unescape(title.text.strip()) if title else 'No Title'),
                km=translate_and_uppercase(html.unescape(km.text.strip()) if km else 'No KM data'),
                engine=translate_and_uppercase(html.unescape(engine.text.strip()) if engine else 'No Engine data'),
                gearbox=translate_and_uppercase(html.unescape(gearbox.text.strip()) if gearbox else 'No gearbox'),
                fuel=translate_and_uppercase(html.unescape(fuel.text.strip()) if fuel else 'No fuel'),
                year=translate_and_uppercase(html.unescape(year.text.strip()) if year else 'No year'),
                price=translate_and_uppercase(html.unescape(price.text.strip()) if price else 'No Price'),
            )
